...
GUIでのデータ処理は、明確な2つのステップで行います。

#### ステップ1: スキーマ分析とマスタ更新
このステップの目的は、**「どのファイルの、どの列を、どのデータ型にするか」という設計図（スキーマ）を確定させること**です。

1.  画面左側のサイドバーにある「**パターンルール管理**」セクションで、データ型の判定ルールを必要に応じて編集します。
    *   **未登録ファイルルール**: 特殊な区切り文字やエンコーディングを持つファイルを定義します。
    *   **日付パターンルール**: 日付と判定したい文字列の**見た目**のパターン（正規表現）を定義します。
    *   **日付フォーマット管理**: パターンに一致した文字列を**実際に日付として解釈**するためのフォーマット（例: `%Y-%m-%d`）を定義します。
    *   **ビジネスロジックルール**: 列名に特定のキーワードを含む場合のデータ型を定義します。

2.  メイン画面の「**ステップ1: スキーマ分析とマスタ更新**」にある「**最新ルールを適用してマスタを更新**」ボタンをクリックします。

3.  これにより、`analyzer.py`が実行され、以下の処理が行われます。
    *   全ての入力ファイルをスキャンし、ルールに基づいて各列のデータ型を決定します。
    *   決定したスキーマ情報が、データベース内の`column_master`テーブルに保存されます。
    *   処理結果として、`output/analysis_report.csv`という単一の公式なレポートが出力されます。このCSVを見れば、どの列が最終的にどのデータ型と判断されたかを確認できます。

#### ステップ2: データをテーブルにロード
このステップの目的は、**ステップ1で確定した設計図（`column_master`）通りに、実際のデータを入れるためのテーブルを作成し、データを流し込むこと**です。

1.  メイン画面の「**ステップ2: データをテーブルにロード**」にある「**データをテーブルにロード**」ボタンをクリックします。

2.  これにより、`loader.py`が実行され、以下の処理が行われます。
    *   `column_master`の定義を読み取り、それと全く同じスキーマのテーブルをデータベースに作成します。
    *   各ファイルからデータを読み込み、対応するテーブルにデータを挿入します。

この2ステップのプロセスにより、「スキーマの定義」と「データのロード」が明確に分離され、信頼性の高いデータ格納が保証されます。
